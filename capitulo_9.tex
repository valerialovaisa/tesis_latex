...
\newpage
	...
	mencionada, un módulo UART y un módulo ETHERNET.
		...
		El Prototipo Dos será evaluado de forma similar al anterior debido a que el cambio del SoC instanciado requiere nuevamente la verificación de los
...
\end{lstlisting}
		...
		Con el prototipo tres se verficó el correcto funcionamiento y soporte para sistemas de tiempo real sobre el microprocesador OpenRISC 1200. Mediante
		...
		operativo, puede conformase una solución integral, efectiva para aplicaciones de tiempo real de código abierto.
...
La implementación de sistema operativo de Linux ser realiza en una serie de etapas detalladas a continuación. 
		\begin {itemize}
		\item Descarga del código fuente del proyecto
		...
		Consulte la Guía de Linux en el Anexo ~\ref{app:apendice5} para una comprensión más profunda de los pasos a seguir en la implementación sobre la
 		placa de desarrollo S3ADSP1800A.
...
\end{lstlisting}
		...
		\hline	 		   Resultados Obtenidos & Al ejecutarse el programa de prueba no pudo observarse la salida salida esperada\\
		\hline	
		\end{tabular}
		...
		\hline	 		   Resultados Obtenidos & Al ejecutarse el Kernel del SO pudo observarse la salida esperada, mostrada en la Referencia
		~\ref{lst:sallinux}\\
		\hline	
		\end{tabular}
...
\end{lstlisting}		
		...
		\hline	\multicolumn{2}{>{\columncolor[gray]{.8}}c}{Resultados}\\
		\hline			   Resultados Esperados & Lograr observar por la terminal serial la secuencia de arranque del Sistema Operativo\\
		\hline	 		   Resultados Obtenidos & Al ejecutarse el Kernel del SO pudo observarse la salida mostrada en la Referencia ~\ref{lst:sallinux1}\\
		\hline	
		\end{tabular}
		\caption{Caso de prueba T014}
		\end{table}
		
\begin{lstlisting}[frame=single,caption={Salida de la secuencia de incio de Linux Embebido},label={lst:sallinux1}]
[ 0.000000] Compiled-in FDT at c02a8820 
[ 0.000000] Linux version 3.9.0-dirty (cudar73@cudar73-desktop) (gcc version 4.5.1-or323 
...
[ 0.000000] Memory: 126088k/131072k available (2270k kernel code, 4984k reserved, 345k ) 
...
[ 1.300000] Unable to handle kernel NULL pointer dereference at virtual address 0x000002 
\end{lstlisting}	
...
%\end{lstlisting}		
		...
		\section{Conclusión}
		El prototipo cuatro ayudó a verificar la posibilidad de implementación del sistema operativo de código abierto más importante en Sistemas Embebidos.
		...
		Se verficó el correcto funcionamiento del gestor de arranque uboot, uno de los más conocidos y verificados. Este hecho posibilitó ejecutar desde el
		...
		resultados satisfactorios. Si se pretende obtener solición a este problema será necesario realizar un análisis exhaustivo al módulo IP SPI
		implementado en el proyecto ORPSoC.
		
		El kernel de Linux pudo compilarse éxitosamente, con herramientas de compilación cruzada, sin mayores inconvenientes durante la construcción su
		imágen y sus herramientas (Busybox). Luego, se ejecutó el kernel sobre el simulador de la arquitectura OpenRISC conocido como or1ksim. Pudo comprobarse
		así, el correcto funcionamiento de la imágen construida y su correcta implementación para una arquitectura OpenRISC. Sin embargo, al intentar
		ejecutar el Kernel sobre el kit de desarrollo S3ADSP1800A, surgieron inconvenientes respecto del acceso a memoria durante la secuencia de inicio del
		Sistema Operativo. En el análisis efectuado ante este problema se presentaron diversos impedimientos. El primero de ellos fue la escasa
		documentación que brinda el proveedor de la placa de desarrollo, que en ocaciones no funciona correctamente. En este trabajo se recurrió a distintas
		notas de aplicación proporcionadas por Xilinx sin lograr resultados satisfactorios. Como alternativa que permita resolver este problema se recurrió
		a la documentación, foros y mail lists de las comunidades Opensource. Es importante destacar que los tiempos de respuesta en este tipo de
		canales no se corresponden con los tiempos de ejecución de este trabajo y mucho menos con los tiempos de la industria. Finalmente, debe realizarse
		un profundo estudio sobre código del kernel que permita solucionar los problemas asociados a la adapatación del SO a la placa de desarrollo. Si lo
		mencionado anteriormente no logra resolver el problema, deberá realizar un estudio de depuración a nivel de hardware. Existen resultados
		satisfactorias que demuestan el correcto funcionamiento del SO en esta arquitectura sobre otras placas de desarrollo, tales como Atlys de Xilinx y
		DE-Nano de Terasic ~\cite{rte.se} ~\cite{denano}.
